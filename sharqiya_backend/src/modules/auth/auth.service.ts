...
function parseTtlMs(ttl: string): number {
  ...
  if (!match) return 7 * TTL_UNITS.d;
  return Number(match[1]) * (TTL_UNITS[match[2]] ?? TTL_UNITS.s);
}
...
export class AuthService {
  ...
  private async issueTokens(user: UserDocument): Promise<LoginResult> {
    ...
    const refreshExpiresAt = new Date(
      Date.now() +
        parseTtlMs(this.config.get<string>('JWT_REFRESH_TTL') ?? '7d'),
    );
  ...
  /** Rotation: the presented token is consumed and a fresh one issued. */
